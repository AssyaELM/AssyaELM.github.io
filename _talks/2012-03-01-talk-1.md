...
Sur chaque switch les ports ont été configurés selon deux modes :
•	Access Mode : Les ports connectés directement aux postes de travail (PC, imprimantes) sont configurés en mode accès..
•	Trunk Mode : Les ports qui relient les switchs aux routeurs  sont configurés en mode trunk. Un port trunk est capable de transporter le trafic de plusieurs VLANs simultanément en utilisant l'étiquetage 802.1Q. C'est essentiel pour permettre au routeur de gérer la communication entre les différents VLANs. Les trames sont étiquetées (tagged) avec l’ID du VLAN pour différencier les VLANs.
```bash
Switch(config)#int range fa0/1
Switch(config-if-range)#switchport mode trunk
Switch(config-if-range)#do wr
Mêmes configurations pour les autres switches.
```
